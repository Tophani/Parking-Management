    ...
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
        <div class="card">
            ...
            <div class="body">
                <div class="row">
                    <div class="col-sm-12"> 
                        {!! Form::open(['url' => 'admin/price/edit', 'class' => 'form-validation']) !!}
                            
                            {{ Form::hidden('p_id', Request::segment(4) ) }}
                            {{ Form::hidden('f_id', Request::segment(5) ) }}

                            <label for="place_id">{{ Lang::label('Parking Zone') }} *</label>
                            <div class="form-group">
                                <div class="form-line  {{ $errors->has('place_id') ? 'error focused' : '' }}">
                                    {{ Form::select('place_id', $placeList,  (old('place_id')?old('place_id'):$price->place_id), ['class'=>'form-control', 'id'=>'place_id', 'placeholder'=>Lang::label('Select Option')]) }} 
                                </div>
                                ...
                                <div class="row priceContent">
                                    <div class="col-sm-2">
                                        <div class="form-group">
                                            <div class="form-line  {{ $errors->has('vehicle_type_id') ? 'error focused' : '' }}">
                                                {{ Form::select('vehicle_type_id[]', $vehicleTypeList, $price->vehicle_type_id, ['class'=>'form-control no-select', 'id'=>'vehicle_type_id', 'placeholder'=>Lang::label('Select Option'), 'required'=>'required']) }} 
                                            </div>
                                    ...
                                    <div class="col-sm-2">
                                        <div class="form-line  {{ $errors->has('time') ? 'error focused' : '' }}">
                                            <input name="time[]" type="number" id="time" class="form-control" placeholder="{{ Lang::label('Time') }}" value="{{ (old('time')?old('time'):$price->time) }}" min="1" required>
                                        </div>
                                        @if ($errors->has('time'))
                                    ...
                                    <div class="col-sm-2">
                                        <div class="form-line  {{ $errors->has('unit') ? 'error focused' : '' }}">
                                            {{ Form::select('unit[]', $unitList,   (old('unit')?old('unit'):$price->unit) , ['class'=>'form-control no-select', 'id'=>'unit', 'placeholder'=>Lang::label('Select Option'), 'required'=>'required']) }} 
                                        </div>
                                        @if ($errors->has('unit'))
                                    ...
                                    <div class="col-sm-2">
                                        <div class="form-line  {{ $errors->has('price') ? 'error focused' : '' }}">
                                            <input name="price[]" type="text" id="price" class="form-control" placeholder="Price" value="{{ (old('price')?old('price'):$price->price) }}" required>
                                        </div>
                                        @if ($errors->has('price'))
                            ...
                            <div class="form-group">
                                <div class="form-line  {{ $errors->has('note') ? 'error focused' : '' }}">
                                    <textarea name="note" type="text" id="note" class="form-control" placeholder="{{ Lang::label('Note') }}">{{ (old('note')?old('note'):$price->note) }}</textarea>
                                </div>
                                @if ($errors->has('note'))
